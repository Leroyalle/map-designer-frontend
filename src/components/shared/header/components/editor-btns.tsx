...
export const EditorBtns: React.FC = () => {
  ...
  const handlePublish = () => {
    ...
    const canvasItems = canvas.getObjects().filter((e) => e.name !== 'background');
    console.log('ITEMS BEFORE PUSH', canvasItems);
    const items: CanvasProjectItem[] = canvasItems.map((obj) => ({
